...
scrna <- readRDS("~/dsdata/projects/Morrisey/Kazu/ligation/Integrate/Seurat_subset.RDS")


scrna$celltypel1_sample <- factor(scrna$celltypel1_sample, c("AT2_wt","AT2_ligation","AT1_wt","AT1_ligation"))

scrna$cellcolorsl1 <-  plyr::mapvalues(scrna$celltypel1_sample,
                                       c('AT1_ligation','AT1_wt','AT2_ligation','AT2_wt'), 
...
#scrna <- RunUMAP(scrna, reduction = "pca", dims = 1:10, n.neighbors = 40,min.dist = .3,verbose = FALSE)
...
scrna$sample <- factor(scrna$sample, levels=c('wt','ligation'))
...
png(filename=paste0(plotdir,'Corrplot_full_circle.png'),width = 10,height = 8,units = 'in',res=300)
corrplot(sp, order = 'hclust', addrect = 2,addCoef.col = 'grey50',tl.col = "black", tl.srt = 45)
dev.off()

png(filename=paste0(plotdir,'Corrplot_full_pie.png'),width = 10,height = 8,units = 'in',res=300)
corrplot(sp, method='pie',order = 'hclust', addrect = 2,addCoef.col = 'grey50',tl.col = "black", tl.srt = 45)
dev.off()
